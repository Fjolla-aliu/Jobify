...
import AddJobFormUser from "../components/AddJobFormUser";
...
import EditJobFormUser from "../components/EditJobFormUser";
...
export default function Me() {
  ...
  useEffect(() => {
    if (deleteJob !== undefined && deleteJob !== "" && deleteJobSecure) {
      axios
        .delete("http://localhost:4000/works/" + deleteJob)
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:4000/works/my/" + user.id)
      .then(function (response) {
  ...
  const onSubmitRegisterJobForm = (data) => {
    const aJob = {
      id: uniqueid,
      title: data.jobTitle,
      technologies: data.jobTechnologies,
    ...
    axios
      .post("http://localhost:4000/works/", aJob)
      .then(function () {
        router.reload();
      })
      .catch(function (error) {
  ...
  const onSubmitEditJobForm = (data) => {
    ...
    axios
      .put("http://localhost:4000/works/" + editJob.id, aJob)
      .then(function () {
        router.reload();
      })
      .catch(function (error) {
        ...
        <div className="w-full md:w-3/5 lg:w-3/4 xl:w-1/2 gap-y-8 border border-tertiaryBackground shadow-2xl rounded-xl overflow-hidden bg-slate-300 rounded-r-lg rounded-b-lg flex-col py-8 xl:py-12 px-6 xl:px-12">
          ...
          <ul className="w-full grid grid-cols-1 xl:grid-cols-2 gap-4">
            {jobs.map((item, index) => (
              ...
              >
                <Link href={"/worker/interviews/" + item.id}>
                  <a>
